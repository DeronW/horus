...
class Horus {
    ...
    _setOption(k, v) {
        if (Object.prototype.hasOwnProperty.call(this.opt, k)) {
            this.opt[k] = v;
        } else {
    ...
    setConfig(key, value) {
        let options = typeof key === "object" ? key : { [key]: value };
    ...
    decorator(event_type, data) {
        ...
        // 旧字段, 需要兼容
        if (schema.custom.eventType)
            schema.event_type = schema.custom.eventType;
